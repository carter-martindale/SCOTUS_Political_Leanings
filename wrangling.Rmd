...
read_petition_total <- function(x, y) {
  z <- read_excel(x, skip = 1) %>% 
    ...
    mutate(year = y)
  # z[2:5, c(1, 3:6, 8)]
  w <- z[c(7:10, 12:15, 17:20, 22:25, 27:30, 32:35, 37:40, 42:45, 47:50, 52:55, 57:60, 62:65), c(1, 3:6, 8)]
  w %>%
    mutate(district = c(rep("DC", 4), rep("1st", 4), rep("2nd", 4), rep("3rd", 4), rep("4th", 4), rep("5th", 4), rep("6th", 4),
                        rep("7th", 4), rep("8th", 4),
                        rep("9th", 4), rep("10th", 4),
                        rep("11th", 4)))
}

# 2016 - 2019 are good using this formula
...
read_p <- function(x, y) {
  z <- read_excel(x, skip = 9,
    ...
    mutate(year = y)
  w <- z[c(7:10, 12:15, 17:20, 22:25, 28:31, 33:36, 38:41, 43:46, 48:51, 54:57, 59:62, 64:67), c(1, 3:6, 8)]
...
# 2005 - 2015 can be read in this way, with two exceptions
# addressed below
...
read_2008 <- function(x, y) {
  z <- read_excel(x, skip = 9,
    ...
    rename("nature_of_proceeding" = "of_proceeding") %>% 
    mutate(year = y)
  w <- z[c(7:10, 12:15, 17:20, 22:25, 28:31, 33:36, 38:41, 43:46, 48:51, 53:56, 58:61, 63:66), c(1, 3:6, 8)]
...
roberts_circuit <- roberts_circuit %>%   
  mutate(percent_granted = as.numeric(granted)/as.numeric(filed),
         percent_denied = as.numeric(denied)/as.numeric(filed))
